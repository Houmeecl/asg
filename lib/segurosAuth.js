...
db.exec(`
    CREATE TABLE IF NOT EXISTS seg_operaciones_nico (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        admin_id INTEGER NOT NULL,
        rut TEXT NOT NULL,
        empresa TEXT NOT NULL,
        nico_account_id TEXT,
        nico_lead_id TEXT,
        sector TEXT, zona TEXT, localidad TEXT, riesgo TEXT,
        monto INTEGER, meses INTEGER, sensibilidad INTEGER,
        prima REAL, umbral REAL,
        payload TEXT NOT NULL,
        estado TEXT NOT NULL,
        error TEXT,
        creada DATETIME DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (admin_id) REFERENCES seg_usuarios(id) ON DELETE CASCADE
    );
`);
...
// ---- Bitácora del onboarding real con Nico (estados: SIMULADA | CUENTA_CREADA | ENVIADA | ERROR) ----
export function registrarOperacionNico(o) {
    const r = db.prepare(`
        INSERT INTO seg_operaciones_nico
          (admin_id, rut, empresa, nico_account_id, nico_lead_id, sector, zona, localidad, riesgo, monto, meses, sensibilidad, prima, umbral, payload, estado, error)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(o.adminId, o.rut, o.empresa, o.cuentaId == null ? null : String(o.cuentaId), o.leadId == null ? null : String(o.leadId), o.sector, o.zona, o.localidad, o.riesgo,
        o.monto, o.meses, o.sensibilidad, o.prima, o.umbral, JSON.stringify(o.payload), o.estado, o.error ?? null);
    return Number(r.lastInsertRowid);
}

export function actualizarOperacionNico(id, campos) {
    const permitidos = { estado: 'estado', error: 'error', cuentaId: 'nico_account_id', leadId: 'nico_lead_id' };
    for (const [k, col] of Object.entries(permitidos)) {
        if (k in campos) db.prepare(`UPDATE seg_operaciones_nico SET ${col} = ? WHERE id = ?`).run(campos[k] === undefined ? null : String(campos[k]), id);
    }
}

export const obtenerOperacionNico = (id) => db.prepare(`SELECT * FROM seg_operaciones_nico WHERE id = ?`).get(id) ?? null;

export function listarOperacionesNico() {
    return db.prepare(`
        SELECT o.id, o.rut, o.empresa, o.nico_account_id, o.nico_lead_id, o.riesgo, o.monto, o.estado, o.error, o.creada, u.email AS admin_email
        FROM seg_operaciones_nico o JOIN seg_usuarios u ON u.id = o.admin_id
