...
const input = 'rounded-xl border border-[#0b1a12]/15 bg-white px-4 py-3 text-sm w-full';
...
export default function Onboarding() {
  ...
  return (
    <div>
      <p className="text-xs uppercase tracking-widest font-semibold text-[#0b1a12]/50">Onboarding demo · paso {paso + 1} de {PASOS.length}</p>
      <h1 className="mt-2 text-3xl md:text-4xl font-semibold tracking-tight">{PASOS[paso]}</h1>
      <div className="mt-5 flex gap-1.5" aria-hidden>
        {PASOS.map((p, i) => <span key={p} className={`h-1.5 flex-1 rounded-full ${i <= paso ? 'bg-[#0b1a12]' : 'bg-[#0b1a12]/10'}`} />)}
      </div>

      <div className="mt-8 rounded-3xl bg-white border border-[#0b1a12]/10 p-6 md:p-8 flex flex-col gap-5">
        {paso === 0 && (
          <>
            ...
            <div>
              ...
              <div className="mt-2 grid sm:grid-cols-2 gap-2">
                {SECTORES.map((s) => (
                  <button type="button" key={s.slug} onClick={() => elegirSector(s.slug)} className={`text-left rounded-xl border px-4 py-3 ${f.sector === s.slug ? 'border-[#0b1a12] bg-[#d7ff3f]/50' : 'border-[#0b1a12]/15 hover:border-[#0b1a12]'}`}>
                    <span className="font-medium text-sm">{s.nombre}</span>
                    <span className="block text-xs text-[#0b1a12]/55 mt-0.5">{s.corto}</span>
                  </button>
                ))}
              </div>
            </div>
        ...
        {paso === 1 && (
          <>
            ...
            <p className="text-xs text-[#0b1a12]/55">En la versión real asignamos aquí la estación meteorológica de referencia más cercana a tu ubicación.</p>
          </>
        )}
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {sector.riesgos.map((k) => (
                  <button type="button" key={k} onClick={() => set('riesgo', k)} className={`px-4 py-2 rounded-full text-sm font-medium border ${f.riesgo === k ? 'bg-[#0b1a12] text-[#d7ff3f] border-[#0b1a12]' : 'border-[#0b1a12]/15 hover:border-[#0b1a12]'}`}>
                    {RIESGOS[k].nombre}
                  </button>
                ))}
              </div>
              <p className="mt-2 text-xs text-[#0b1a12]/55">{sector.riesgoDetalle[f.riesgo]}</p>
            </div>
            <label className={label}>Monto asegurado: <span className="normal-case tracking-normal text-sm text-[#0b1a12]">{clp(f.monto)}</span>
              <input type="range" min={5_000_000} max={500_000_000} step={5_000_000} value={f.monto} onChange={(e) => set('monto', +e.target.value)} className="accent-[#0b1a12]" />
            </label>
            <div className="grid sm:grid-cols-2 gap-4">
              <label className={label}>Periodo: {f.meses} meses
                <input type="range" min={1} max={12} value={f.meses} onChange={(e) => set('meses', +e.target.value)} className="accent-[#0b1a12]" />
              </label>
              <label className={label}>Sensibilidad: {f.sens}%
                <input type="range" min={10} max={100} value={f.sens} onChange={(e) => set('sens', +e.target.value)} className="accent-[#0b1a12]" />
              </label>
            </div>
          </>
        )}
          ...
          <>
            <dl className="grid sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
              {[['Empresa', f.empresa], ['Sector', sector.nombre], ['Ubicación', `${f.localidad}, ${f.zona}`], ['Riesgo', r.nombre], ['Monto asegurado', clp(f.monto)], ['Periodo', `${f.meses} meses`]].map(([k, v]) => (
                <div key={k}><dt className="text-xs uppercase tracking-wider text-[#0b1a12]/50 font-semibold">{k}</dt><dd className="mt-0.5">{v}</dd></div>
              ))}
            </dl>
            <div className="rounded-2xl bg-[#0b1a12] text-white p-6">
              <p className="text-xs uppercase tracking-widest text-[#d7ff3f]">Prima indicativa</p>
      ...
      <div className="mt-6 flex justify-between">
        ...
        {paso < PASOS.length - 1 ? (
          <button type="button" onClick={() => setPaso((p) => p + 1)} disabled={!puedeAvanzar} className="flex items-center gap-2 rounded-full bg-[#0b1a12] text-[#d7ff3f] px-6 py-3 text-sm font-semibold disabled:opacity-40">
