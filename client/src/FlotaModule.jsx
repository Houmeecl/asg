...
export default function FlotaModule() {
  ...
  useEffect(() => {
    if (empresaSeleccionada) {
      cargarClientes(empresaSeleccionada.id);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col gap-4">
          ...
          <div className="flex-1 overflow-y-auto space-y-2 max-h-72">
            {empresas.map(emp => (
              <div key={emp.id} onClick={() => setEmpresaSeleccionada(emp)}
                className={`p-3 rounded-lg border cursor-pointer transition-all ${empresaSeleccionada?.id === emp.id ? 'bg-emerald-950/30 border-emerald-500' : 'bg-slate-950 border-slate-800 hover:border-slate-700'}`}>
            ...
            {empresas.length === 0 && <p className="text-xs text-slate-500 text-center py-4">No hay empresas registradas.</p>}
          </div>
        </div>
          ...
          {empresaSeleccionada ? (
            <>
              ...
              <div className="flex-1 overflow-y-auto space-y-2 max-h-56">
                {contratos.map(c => (
                  <div key={c.id} onClick={() => setContratoSeleccionado(c)}
                    className={`p-2.5 rounded-lg border cursor-pointer transition-all ${contratoSeleccionado?.id === c.id ? 'bg-blue-950/30 border-blue-500' : 'bg-slate-950 border-slate-800 hover:border-slate-700'}`}>
                ...
                {contratos.length === 0 && <p className="text-xs text-slate-500 text-center py-2">Sin contratos aún.</p>}
              </div>
            </>
          ) : (
          ...
          {contratoSeleccionado ? (
            <div className="flex-1 flex flex-col gap-3 overflow-y-auto">
              ...
              <form onSubmit={handleRenovar} className="space-y-2 bg-slate-950/50 p-3 rounded-lg border border-slate-800">
                <label className="text-[10px] text-slate-500 uppercase">XML de uso (distancia/horómetro)</label>
                <textarea rows="5" value={xmlUso} onChange={e => setXmlUso(e.target.value)}
                  className="w-full bg-slate-900 border border-slate-700 rounded p-2 text-[10px] font-mono text-slate-300" />
                <label className="text-[10px] text-slate-500 uppercase">XML de combustible</label>
                <textarea rows="6" value={xmlCombustible} onChange={e => setXmlCombustible(e.target.value)}
                  className="w-full bg-slate-900 border border-slate-700 rounded p-2 text-[10px] font-mono text-slate-300" />
              ...
              {certificado && (
                <div className="bg-slate-950 border border-emerald-500/40 rounded-lg p-3 flex flex-col gap-2">
                  <div className="flex items-center gap-3">
                    <img src={certificado.qr_data_url} alt="QR certificado" className="w-20 h-20 rounded bg-white p-1" />
